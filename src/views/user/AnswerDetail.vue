...
<script>
import { getExamDetail } from "@/api/user";

export default {
  name: "user-answer-detail",
  data() {
    return {
      problem_list: [],
      unit_name: "",
  ...
  async mounted() {
    if (localStorage.getItem("role") === "0") {
      let { data } = await getExamDetail(this.$route.params.unit_id);
      console.log(data);
      let problem_list = data.data.problem_list;
